...
class Agent(object):
    ...
    def play(self, plot_rew=False, lap=0):
        '''
        plays the game
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            saver = tf.train.import_meta_graph('{}/model.ckpt.meta'.format(self.Network.network_path))
    ...
    def policy(self, previous_obs, sess, X_train, keep_prob, out):
        ...
        for _2 in range(4):
            X_feed[0, :, :, _2 * self.parameters['input_C']:(_2 + 1) * self.parameters['input_C']] = previous_obs[_2, :, :, :] / 255
